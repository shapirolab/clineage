...
def challenge_triplets_generator(ref_tree, n=1000, min_d=5):
    ...
    for nodes_triplet in memory_expensive_random_choose(nodes, 3, n=n):
        ref_triplet = ref_tree.extract_tree_with_taxa(nodes_triplet)
        ...
        distance_a_to_ab_mrca = ndm.distance(ref_tree.find_node_for_taxon(taxon_a), ref_tree.mrca(taxa=ref_close_pair))
        distance_b_to_ab_mrca = ndm.distance(ref_tree.find_node_for_taxon(taxon_b), ref_tree.mrca(taxa=ref_close_pair))
...
def check_triplet(tree, nodes_triplet, ref_close_pair):
    ...
    if len(rec_triplet.leaf_nodes()) < 3:
        return None
    rec_pdm = rec_triplet.phylogenetic_distance_matrix()
    rec_pdm = tree.phylogenetic_distance_matrix()
    pairs_in_triplet = list(itertools.combinations(nodes_triplet,2))
    rec_close_pair = set(sorted(pairs_in_triplet, key=lambda x: rec_pdm.path_edge_count(x[0],x[1]))[0])
    return ref_close_pair == rec_close_pair


def triplets_score(tree_in_question, reference_tree, n=10000, min_d=3):
