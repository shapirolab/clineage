...
def populate_conditioned_mpx(conditioned_mpx, target_enrichment):
    ...
    if other_target_enrichment_withing_margins(target_enrichment, conditioned_mpx['target_enrichments']):
        return False, conditioned_mpx
...
def populate_conditioned_mpxs_map(target_enrichment_queryset, conditioned_mpxs):
    ...
    for target_enrichment in target_enrichment_queryset:
        ...
        for conditioned_mpx in conditioned_mpxs.keys():
            added, conditioned_mpxs[conditioned_mpx] = populate_conditioned_mpx(conditioned_mpxs[conditioned_mpx], target_enrichment)
            if added:
                break
        if not added:
