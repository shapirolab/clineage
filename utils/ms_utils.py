...
def extend_ms(ms, k=1):
    if ms.chromosome.getdna(ms.end_pos+1, ms.end_pos+k*ms.repeat_unit_len) == ms.referencevalue.sequence[:ms.repeat_unit_len]:
        ms.repeat_number = floor(ms.repeat_number) + k
        seq = ms.referencevalue.sequence + ms.referencevalue.sequence[:ms.repeat_unit_len]
        sequence = save_sequence(seq)
        ms.end_pos = ms.start_pos + len(sequence.sequence) - 1
        assert ms.chromosome.getdna(ms.start_pos, ms.end_pos) == sequence.sequence
        ms.referencevalue = sequence
...
def resolve_overlapping_mss(te_queryset, max_trim=3):
    ...
    for te in te_queryset:
        for t in te.targets.all():
            ...
            overlapping_mss = Microsatellite.objects.filter(chromosome=ms.chromosome)\
                .filter(start_pos__lte=ms.end_pos)\
                .filter(end_pos__gte=ms.end_pos)\
                .exclude(pk=ms.pk)
            if overlapping_mss:
