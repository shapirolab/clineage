...
def get_rowdict(target, ms, snp, ter, amplicon, panel, mx):
    return {
            ...
            'Chromosome': smart_text(ter.left_primer.ugs.slice.chromosome.name),
            'Length MS': smart_text(abs(ms.slice.end_pos - ms.slice.start_pos) if ms is not None else ''),
            'Target sequence': smart_text(target.slice.sequence.seq.decode('utf-8') if target is not None else ''),
    ...
    for panel in panels:
        ...
        for mx in mxs:
            for ter in mx.ters.select_subclasses():
                ...
                if Target.objects.filter(slice__contained=amplicon.slice).count() == 0:
                    yield get_rowdict(None, None, None, ter, amplicon, panel, mx)
                for target in Target.objects.filter(slice__contained=amplicon.slice):
                    snp = None
                    ms = None
                    try:
                        ...
                        try:
                            ...
                            continue
                    yield get_rowdict(target, ms, snp, ter, amplicon, panel, mx)
