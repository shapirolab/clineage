...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for p in orm.Primer.objects.filter(tail__isnull=False).filter(xtail=None):
            p.xtail = orm.PrimerTail.objects.get(tail=p.tail)
            p.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        for p in orm.Primer.objects.filter(xtail__isnull=False):
            p.xtail = None
            p.save()
